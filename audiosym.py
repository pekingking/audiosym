...
class audiosym():
    ...
    def findBookDetails(self, source):
        ...
        while True:
            ...
            if correctBook == "next":
                ...
                bookSeries = self.input("goodreads id: ", bookDetails["series"])
                bookSeriesNumber = self.input("goodreads id: ", bookDetails["seriesNumber"])
                if bookTitle and bookAuthor and bookID:
                    bookDetails = dict()
                    bookDetails["title"] = bookTitle
                    bookDetails["author"] = bookAuthor
                    bookDetails["goodreadsID"] = bookID
                    bookDetails["series"] = bookSeries
                    bookDetails["seriesNumber"] = bookSeriesNumber
                    bookDetails["date"] = ""
                    bookDetails["description"] = ""
                    bookDetails["thumbnailURL"] = ""
                    return bookDetails
    ...
    def createSymlinks(self, args, bookDetails, scriptPath, orderedFileList, prepend=""):
        if not os.path.exists("{}/{}{}{}:{}".format(args.destination.rstrip('/'), self.cleanTitle(bookDetails["series"]) + ":" + self.cleanTitle(bookDetails["author"]) + "/" if bookDetails["series"] else "", self.cleanTitle(bookDetails["seriesNumber"]) + "." if bookDetails["seriesNumber"] else "", self.cleanTitle(bookDetails["title"]), self.cleanTitle(bookDetails["author"]))):
            ...
            os.makedirs("{}/{}{}{}:{}".format(args.destination.rstrip('/'), self.cleanTitle(bookDetails["series"]) + ":" + self.cleanTitle(bookDetails["author"]) + "/" if bookDetails["series"] else "", self.cleanTitle(bookDetails["seriesNumber"]) + "." if bookDetails["seriesNumber"] else "", self.cleanTitle(bookDetails["title"]), self.cleanTitle(bookDetails["author"])))
